...
> [!IMPORTANT]
> **Architecture Update / 架构更新**: Detailed documentation has been moved to individual sub-directory `.md` files for better maintainability.
> **架构更新**：为了更好的可维护性，详细文档已迁移至各子目录独立的 `.md` 文件中。

## Detailed Sections / 各部分详细文档

*   ### [`Sidebar/` (Left Sidebar / 左侧边栏)](./Sidebar/Sidebar.md)
*   ### [`RightSidebar/` (AI Sidebar / AI 侧边栏)](./RightSidebar/RightSidebar.md)
*   ### [`PDFReader/` (Document View / PDF阅读器视图)](./PDFReader/PDFReader.md)
*   ### [`settings/` (Global Configurations / 全局配置)](./settings/settings.md)
*   ### [`PaperList/` (Library Table / 资料库表格)](./PaperList/PaperList.md)

---

## ~~Structure & Breakdown / 结构与分解 (Legacy / 已迁移)~~

~~### `Sidebar/` (Left Sidebar / 左侧边栏)~~

~~Manages batch imports, local groups, and application basic navigation. / 管理批次导入、本地分组以及应用程序基础导航。~~
...
~~Provides an integrated AI chat interface mapped dynamically to Tauri backends. / 提供了一个动态映射到 Tauri 后端的集成 AI 聊天界面。~~
...
~~Embedded PDF display handling dynamic rendering (`react-pdf`), annotations, jumping, and interactions. / 嵌入式 PDF 显示组件，处理动态渲染（`react-pdf`）、注释、跳转和相关交互。~~
...
(Refer to detailed breakdown inside [settings.md](./settings/settings.md)) / (详情请参阅 [settings.md](./settings/settings.md) 中的拆解说明)

### `PaperList/` (Library Table / 资料库表格)
(Refer to detailed breakdown inside [PaperList.md](./PaperList/PaperList.md)) / (详情请参阅 [PaperList.md](./PaperList/PaperList.md) 中的拆解说明)
